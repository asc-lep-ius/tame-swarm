...
def create_wealth_distribution_plot(
    wealth_trace: dict[str, Any], simplified: bool = False
) -> go.Figure:
    """
    ...
    if not wealth_trace or not wealth_trace.get("expert_wealth"):
        ...
        fig.add_annotation(
            text=(
                "No wealth trace data available yet.<br>"
                "Generate a response to see VCG auction dynamics."
            ),
            xref="paper",
            yref="paper",
            x=0.5,
    ...
    for expert_idx in range(num_experts):
        ...
        fig.add_trace(
            go.Scattergl(  # Use WebGL for faster rendering
                x=steps,
                y=wealth_values,
                mode="lines",
    ...
    if not simplified and expert_wealth:
        ...
        if n > 0 and sum(sorted_wealth) > 0:
            ...
            gini = 0
        inequality_text = (
            f"Gini: {gini:.3f} ({'Specializing!' if gini > 0.2 else 'Low inequality'})"
        )
...
def create_steering_trace_plot(
    steering_trace: dict[str, Any], simplified: bool = False
) -> go.Figure:
    """
    ...
    if not steering_trace or not steering_trace.get("strength_history"):
        ...
        fig.add_annotation(
            text=(
                "No steering trace data available yet.<br>"
                "Generate a response to see homeostatic dynamics."
            ),
            xref="paper",
            yref="paper",
            x=0.5,
    ...
    if not simplified and strength_history:
        ...
        label = "Dynamic!" if variance > 0.01 else "Static - may need tuning"
        dynamics_text = f"Variance: {variance:.4f} ({label})"
...
def check_server_health() -> str:
    ...
    try:
        ...
        if resp.ok:
            data = resp.json()
            mob = "✓" if data["mob_active"] else "✗"
            steer = "✓" if data["steering_active"] else "✗"
            return f"✓ Connected | GPU: {data['gpu']} | MoB: {mob} | Steering: {steer}"
...
def get_swarm_status() -> str:
    ...
    try:
        ...
        if resp.ok:
            ...
            experts = [(i, w, u) for i, (w, u) in enumerate(zip(wealth, usage, strict=True))]
            experts.sort(key=lambda x: x[2], reverse=True)

            lines = [
                f"**Expert Swarm Status** "
                f"({data['num_experts']} experts, {data['layers_modified']} layers)"
            ]
            ...
            try:
                ...
                if event_type == "token":
                    ...
                    if usage:
                        ...
                        stats_parts.append(f"Tokens: {in_tok} → {out_tok}")
    ...
    with gr.Blocks(title="TAME Swarm Chat") as demo:
        ...
        with gr.Row():
            status_box = gr.Textbox(
                label="Server Status", value=check_server_health(), interactive=False, scale=4
            )
        ...
        with gr.Row():
            ...
            with gr.Column(scale=3):
                ...
                with gr.Row():
                    msg_input = gr.Textbox(
                        label="Message", placeholder="Type your message here...", scale=4, lines=2
                    )
                    send_btn = gr.Button("Send", variant="primary", scale=1)

                stats_display = gr.Textbox(
                    label="Generation Stats", interactive=False, visible=True
                )
            ...
            with gr.Column(scale=1):
                ...
                max_tokens = gr.Slider(
                    minimum=32, maximum=4096, value=512, step=32, label="Max Tokens"
                )
        ...
        *Good sign: Dynamic - should spike when model drifts from goals!*
        ...
        def respond_stream(message, history, temp, max_tok, steer, stats):
            ...
            for response_text, stats_text, w_trace, s_trace in stream_chat(
                message, history, temp, max_tok, steer, stats
            ):
                ...
                has_new_trace_data = (wealth_data_len > last_wealth_len) or (
                    steering_data_len > last_steering_len
                )
...
if __name__ == "__main__":
    demo = create_ui()
    demo.launch(server_name="0.0.0.0", server_port=7860, share=False, theme=gr.themes.Soft())  # pyright: ignore[reportPrivateImportUsage] # gradio stubs don't export themes
