...
- **Routing share.** Under the default `routing_share="uniform"` each winner takes exactly `1/top_k`, which is what makes the auction strategyproof: a share that rose with a winner's own report while its VCG price did not would be output influence bought for free. `"proportional"` restores an own-bid-weighted gate as the [#12](../README.md#phase-05--mechanism-correction) comparison arm, taken in the log domain — a winner's share is `bid ** (1 / routing_temperature)` normalised over the winners — so a uniform rescaling of all wealth leaves the weights unchanged and only *relative* wealth reaches the gate. A softmax over the raw bids does not have that property, and at the configured wealth band it collapsed to an effective expert count of 1.000.
...
5. **Cavallo rebates** return part of what the auction collected — each expert is rebated from the (k+1)-th highest bid *among the others*, divided by the pool's largest wealth, so the payout is report-independent and affordable in credits. How much comes back depends on the wealth spread: ~94% when wealth is flat, ~68% across the configured band, under 4% in a `max_wealth` monopoly. See [#15](../README.md#phase-05--mechanism-correction).
