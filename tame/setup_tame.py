...
def check_dependencies():
    ...
    try:
        from mob import MixtureOfBidders, MoBConfig  # noqa: F401
...
def run_training(args):
    ...
    logger.info(
        ...
        config.num_experts,
        config.top_k,
        config.mob_layers_start,
        config.mob_layers_end,
...
def main():
    ...
    from config import ACTIVE_MODEL, get_active_profile
...
Examples:
  ...
  python setup_tame.py --mode full --steps 5000
    ...
    parser.add_argument(
        "--batch_size", type=int, default=2, help="Batch size per step (default: 2 for 16GB GPU)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--num_experts", type=int, default=4, help="Number of experts (must match main.py)"
    )
    parser.add_argument(
        "--top_k", type=int, default=2, help="Experts per token (must match main.py)"
    )
    parser.add_argument(
        "--mob_layers_start",
        type=int,
    ...
    # Export
    parser.add_argument(
        "--checkpoint", type=str, default=None, help="Checkpoint path for export mode"
    )
    parser.add_argument("--export_dir", type=str, default="./tame_inference")

    # Misc
    parser.add_argument(
        "--cleanup", action="store_true", help="Clean up test files after test mode"
    )
