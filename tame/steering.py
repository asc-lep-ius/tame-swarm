...
class SteeringHook:
    ...
    def _project_orthogonal(self, vector: torch.Tensor, subspace: torch.Tensor) -> torch.Tensor:
        result = vector.clone()
    ...
    def attach_to_model(self, model: nn.Module):
        ...
        for layer_idx, steering_vector in self.steering_vectors.items():
            ...
            hook_obj = SteeringHook(
                steering_vector=steering_vector, config=self.config, homeostat=self.homeostat
            )
