...
def body_calibration(layers=BODY_LAYERS, seed: int = 7, lift: float = 0.6) -> AlignmentCalibration:
...
def paid_body(goals=(), doses=(), calibrations=None, seed: int = 0, device=None) -> nn.Module:
    ...
    and have to reach the device their layer ended up on.
    """
    ...
    if goals:
        attach_goal_fields(model, goals, doses, calibrations or {})
    return model
