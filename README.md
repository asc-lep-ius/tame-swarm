...
- **What is not claimed.** No number here is from a real model, and whether coupling improves routing on Qwen3-1.7B is the ablation #6 exists to run. In TAME terms the field now reaches the cells, at the layers where a gradient was measured, through a receptor each cell learns for itself — competence without comprehension — and its effect on the organism is a measurement still owed.
...
| Morphogenetic field shaping cell behaviour | The goal direction shifts what each confidence head perceives, through a receptor each layer learns, only at layers where the direction is certified | Implemented, opt-in ([#14](#coupling-activation-14)); effect unmeasured until [#6](#phase-1--steering-economy-coupling) |
...
| Task | Description | Status |
|------|-------------|--------|
| **1a. Inject steering into confidence** | Perception modulation (#2): the hidden state each confidence head reads is shifted along the goal direction by a learned receptor, $h' = h + \beta\,(u \cdot h)\,d$, so experts that read the goal direction as valuable bid higher on their own and the auction runs unchanged | Done as a mechanism ([#14](#coupling-activation-14)): seeded by the trainer from the certified direction at the certified layers, opt-in; its effect on routing quality is the ablation [#6](#phase-1--steering-economy-coupling) measures |
