...
**The wealth band, re-derived (#16).** `initial_wealth=75`, `min_wealth=15`, `max_wealth=750` and `wealth_decay=0.997` were fitted against a gate that no longer exists. [#11](#phase-05--mechanism-correction) took wealth out of gate sharpness, so what the band still shapes is selection (winners are `argtopk(confidence × wealth)`, so the band's *ratio* is how much better a poor expert's report must be to overturn a rich one), price magnitude (`b_(k+1) / w_j`) and rebate size. `scripts/sweep_wealth_bounds.py` sweeps the ratio against the decay — the band centred geometrically, because the gate reads log-wealth — plus a pass over the absolute scale, which the gate cannot see but prices can. **All four constants are retained, and none by assumption.**

*The ledger is a leaky integrator, and that is what fixes the scale.* Wealth moves by `w ← decay·w + net`, so it forgets with a time constant of `1/(1 − decay)` steps and settles at `w* ≈ net/(1 − decay)`. The band has to contain that equilibrium, and swept over two orders of magnitude at ratio 10 and decay 0.99, **75 is the only scale where it lands inside**:

| `initial_wealth` | band | ceiling occupancy | floor occupancy | floor's share of all wealth | Gini | wealth overturns |
|---:|---|---:|---:|---:|---:|---:|
| 7.5 | [2.4, 23.7] | **96.0%** | 0.0% | 0.00 | 0.000 | 1.7% |
| 25 | [7.9, 79.1] | 59.0% | 0.0% | 0.00 | 0.001 | 4.9% |
| **75 (shipped)** | **[23.7, 237.2]** | **18.1%** | **17.3%** | **0.18** | **0.518** | **26.3%** |
| 250 | [79.1, 790.6] | 0.0% | 44.9% | 0.36 | 0.403 | 22.0% |
| 750 | [237.2, 2371.7] | 0.0% | **58.3%** | **0.78** | 0.091 | 16.4% |

*The ceiling cannot be a cap on inequality.* Every band whose ceiling binds hard enough to reduce how often wealth overturns reports does so by pinning **every** expert against it: at `[37.5, 150]` with the shipped decay, ceiling occupancy is 82% and wealth Gini is **0.000**. The ceiling erases the ledger rather than compressing it. What it *is* for is numerical — it is the largest wealth at which the auction's payment properties are asserted to hold, and the property tests draw their markets log-uniformly from exactly this band.

*The decay is bounded above and below by two different tests.* At 1.0 the ledger never forgets and the market stops re-forming — `test_the_market_re_forms_around_a_senescent_expert` and the forced-routing recovery both fail. At 0.995 or 0.99, drainage alone removes a senescent expert, so `test_frozen_heads_leave_the_senescent_expert_in_the_market` fails: the pairing that attributes removal to the *value objective* collapses, because decay has quietly taken over its job. 0.997 is a memory horizon of 333 steps and sits between the two.

**What mutating each constant kills**, running the whole CPU suite once per mutant. `test_default_values_match_expected` pins the literal values and fires on all four, so it is discounted throughout — a value-pinning test inflates a mutation score without saying anything about mechanism:

| mutated | real kills | load-bearing on |
|---|---:|---|
| `max_wealth` 750 → 1e9 | 3 | payment positivity under float32, and both coupling property tests |
| `min_wealth` 15 → 0.001 | **0** | **nothing** — the gap `test_the_floor_bounds_what_a_poor_winner_can_be_charged` now closes |
| `wealth_decay` 0.997 → 1.0 | 2 | recovery from damage |
| `initial_wealth` 75 → 750 | 7 | the price/value crossing, report calibration, forced-routing recovery |
| the clamp itself | 4 | the two boundedness assertions |

That is the reverse of the premise the issue was written on: under the corrected economy **the ceiling is load-bearing and the floor was the inert one**.

**What the band is not.** [#6](#integration-tests-6) recorded two strict expected failures naming this issue, and neither is the band's. Pinning the ledger flat — `min = max = initial`, so a constant multiplier cannot reorder `confidence × wealth` and selection is the report ranking alone, while the auction still prices — leaves the 150-step forced episode still failing: 0.37 of the steady share against the 0.8 required, 1.66× the steady loss against 1.0. The band carries part of the damage (2.64× → 1.66× on the loss, 0.09 → 0.37 on the regained share) and the rank correlation does not improve at all. **No band in the swept space flips either test**, and the ruined expert's win share only clears chance with wealth flat (0.215), being back to 0.063 at a ratio of 4. What remains is the cause the old reason could not exclude — a head learns only from the value its own expert realises, so a lock-out leaves it with a stale report and the uniform 2% exploration slot re-samples it no faster for having been starved longer. That is [#26](https://gitlab.hephaestus/mipkovich/tame-swarm/-/issues/26), and both expected failures now name it.
...
| **`spec/report_decisiveness`** | Context | Fraction of tokens whose top-1 winner is the top-1 report. 1.0 for the `softmax` arm by construction; below 1 for `mob` is wealth overturning reports. Measured at 72.6% on the planted-competence economy at the shipped band ([#16](#wealth-bounds-16), 600 steps, 3 seeds); the 31–33% recorded here before was the pre-#15 economy, where near-identical reports let a 2.0× wealth spread decide almost every token |
...
| `spec/report_decisiveness` far below 1 | Wealth is overturning the reports; the auction's incentive machinery governs less of the routing than it appears to | **Not by narrowing the band** — [#16](#wealth-bounds-16) measured that every band whose ceiling binds hard enough to raise decisiveness does it by pinning every expert against the ceiling, driving wealth Gini to 0.000. Read it beside `expert_cosine_distance` first: near-tied reports let any band decide everything |
| Gini > 0.6, one expert dominates | Wealth monopoly | Raising `min_wealth` narrows the ratio and is the safe half. Lowering `max_wealth` erases the ledger rather than compressing it, and raising `wealth_decay` past 0.997 stops the market re-forming after damage — both measured in [#16](#wealth-bounds-16) |
...
| **#16. Wealth bounds** | Deferred from #11: `[15, 750]` and `decay=0.997` were tuned against a gate that no longer exists and before #9 corrected the payments. Re-derived on the planted-competence economy with `scripts/sweep_wealth_bounds.py` (band ratio × decay, plus the absolute scale) and by mutating each constant against the whole suite. **All four retained, each with a measured reason**: the ledger is a leaky integrator settling at `net/(1 − decay)`, and 75 is the only scale in two orders of magnitude where the band contains that equilibrium; the ceiling is a guard on the payment rule's float32 conditioning and *not* a usable cap on inequality, since every band that binds hard enough to raise report decisiveness drives wealth Gini to 0.000; the decay is bounded above by market re-formation and below by the senescent-expert pairing. The floor turned out to be the inert one — lowering it 15000× broke nothing — which is now fixed by a property test. **The two strict expected failures are re-attributed away from the band to [#26](https://gitlab.hephaestus/mipkovich/tame-swarm/-/issues/26)**: with the ledger pinned flat the forced episode still fails, so what stops recovery is a starved head that the uniform exploration slot never re-samples. See [The wealth band, re-derived](#wealth-bounds-16) | Done (band retained with reasons; held-out reading deferred to #25, where the experts are differentiated enough for it to mean anything) |
...
| **1d. Integration tests & stability validation ([#6](#integration-tests-6))** | Properties from theorems, every test paired with its inert state: hypothesis over the auction, the tissue on the measured plant (which found and fixed a setpoint kick in the derivative), recovery from designed and undesigned damage on a wired tiny system and the real one, the contrastive pipeline end to end, a dead-field scan over every config, the coupling ablation at parity | Done; two recovery claims recorded as expected failures pending [#16](#phase-05--mechanism-correction) |
