...
- **A tissue, not a controller with subordinates.** The pattern is stored distributively — every cell keeps its own direction, resting calibration and setpoint — and the slow state is shared: all cells carry the same integrator, so any surviving cell holds the whole memory. Liveness is a property of firing, not wiring: a cell that misses a pass drops out of the consensus and rejoins when it fires again, so a removed hook is handled by the same rule as normal operation. There is no fallback path and no privileged sensor; the readout above the top actuator is just a cell that votes without injecting.
...
- **The real substrate** (`tests/test_real_model.py`, `gpu`-marked, Qwen3-1.7B from the runner's cache, the served tissue and the coupling seeded at 13, 16–21). The unsteered greedy continuation of one prompt is generated once by a seeded fixture and replayed token by token under each regime with the economy frozen (identical across processes; fresh greedy generations under bf16 kernels are not, and one flipped token rewrites the tail). Content pushed against the certified direction at every actuator block, as the injection itself is applied, leaves the constant-strength loop 1.10 σ below setpoint over the last 60 tokens (−0.01 σ unpushed) and the live loop at 0.15 σ with its strength raised by 0.56 units — on the real plant the criterion is relative, halve the inert error and pay for it in strength, because a 60-token tail of 9-token-filtered readings carries a standard error near 0.4 σ; the top actuator removed halfway through the replay leaves the consensus, and the six survivors raise their strength from 3.94 to 5.05 and hold the error where it was (0.57 σ before, 0.51 after, against 1.10 inert); the `safe` direction passes the behavioural gate on the built-in split against eight random directions and the prefix control. The GPU job runs in 96–98 seconds on the workstation against the 120-second budget; under CI the job **fails** rather than skips when the model is not in the runner's cache, because a skipped real-model suite is a green job that ran nothing (#13), so the workstation runner must mount `~/.cache/huggingface` into the job.
