...
|---|---:|---:|---:|---:|
| base (MoB only) | 65.8 ms | — | 36.0 | — |
| + steering coupling | 67.6 ms | **+2.73%** | 33.8 | −5.9% |
| + routing trace | 66.4 ms | +0.93% | 35.1 | −2.5% |
| + tissue hooks | 66.7 ms | +1.34% | 33.6 | −6.7% |
| all three (the served process) | 69.2 ms | +5.13% | 31.3 | −13.1% |

  The coupling clears #5's budget of 5% of the forward pass with room. Throughput falls further than the forward-pass percentage in every row, and that is the honest shape of it: decode is kernel-launch bound on a model this size, so a per-token addition costs more there than in a 256-token teacher-forced pass. Read the two columns with different precision: across repeated runs the forward-pass percentages move by under 0.2 points, while the throughput deltas move by about 2 — one greedy decode of 3 × 128 tokens per configuration is a single sample, where the forward figure is a median of 20. `coupling_overhead_pct` and both throughput figures are logged to MLflow when a run is open.
