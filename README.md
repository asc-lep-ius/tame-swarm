...
**The shipped band, settled**: 25.0% of expert-steps at the ceiling, 36.4% at the floor, Gini 0.693, mean wealth 198.8, wealth deciding the top-1 winner on 26.6% ± 5.9 of tokens. A stratified economy that touches both bounds — and the floor still does not hold up the mean, accounting for **2%** of all surviving wealth.
...
The settled economy is identical; only how long it takes to first touch the ceiling moves. The equilibrium is an absolute wealth, so what has to contain it is the **band**. Sliding the whole band at a fixed ratio is what changes whether it fits — at ratio 10 and the shipped decay, `[2.4, 23.7]` ends with 99.9% of expert-steps at the ceiling and Gini 0.000, while `[237, 2372]` ends with 74.9% at the floor and the floor propping up 34% of all wealth.

*The ceiling compresses the ledger rather than capping it.* Reducing how often wealth overturns reports always costs the spread: at `[37.5, 150]` overturn falls to 1.0% and Gini with it, to 0.124. The limit case is unambiguous — **at decay 1.0 every band whose bounds differ ends with 100% of expert-steps at the ceiling, Gini exactly 0.000 and overturn 0.0%** — every expert equally and maximally rich. (At ratio 1 the bounds coincide, so the clamp columns report the band's definition rather than the economy's behaviour and are shown as `n/a`.)
...
**The ratio demands more than competence supplies.** The shipped band demands a **50×** report advantage of an expert at the floor. On the planted-competence fixture the advantage competence actually buys is **4.8×** — so wealth outranks competence in selection by an order of magnitude. That is why the ruin claim reads a win share of 0.002, and closing the gap would need a ratio near 5, which is exactly the regime where the ceiling compresses the ledger to nothing.
