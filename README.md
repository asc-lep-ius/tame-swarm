...
- **The tissue on the plant #4 measured** (`tests/test_pid_integration.py`). A simulated tissue with the Qwen3-1.7B calibration (per-cell lifts 0–0.58 σ/unit, reference 4 in [2, 6], the 9-token filter, the two-token consensus delay) reproduces SIMC's `kp = 1.97, ki = 0.22`; a setpoint step settles to 5% in 28 tokens against the bound of `3 × (9 + 2) = 33` with 0.5% overshoot (P-only never settles; four times the derived integral gain — still inside the API's stability bound — overshoots by 27%); PI removes the 0.27 σ residual P control leaves on a 0.5 σ deficit; conditional integration unwinds a pinning deficit in 43 tokens against 199 for a free integrator; the derivative is a quarter as noisy filtered as raw. **The setpoint-kick test found a defect:** the tissue handed its controller `setpoint − consensus` as the process variable, so a setpoint change kicked a derivative that was designed to ignore it — inert in production only because `kd` ships at zero, live the moment `PUT /steering/gains` sets it. The controller now receives the consensus reading against the consensus setpoint; the integral and proportional terms are unchanged, and the first-pass kick is zero against 0.04 with the derivative on the error.
- **Recovery, designed and undesigned** (`tests/test_homeostatic_recovery.py`, on `tests/wired_system.py`: a tiny transformer with MoB blocks, four actuators and a readout, a calibrated tissue and the coupling seeded from the same direction). Content that drags the stream off its setpoint is corrected to within 5% of the tissue setpoint from pass 33 of a 200-pass budget, coupling live or not, and not at all by a loop that senses but cannot act; an actuator removed mid-generation leaves the consensus after one pass, the survivors raise their strength, and the tissue is back in band by pass 50. A property the fixture pinned: content entering *below* the bottom actuator is read by a cell nothing can correct, and the shared integrator zeroes the tissue mean by pushing the regulable cells past their setpoints by that blind error divided by the number of other live cells — a seventh on the served eight-cell tissue. On the planted-competence economy, the distinctly TAME claims: the best expert going **senescent** (still bids, contributes nothing) is removed by its own head learning it is worth nothing — within 200 steps the loss sits within 1.06× the floor a collective born without that expert reaches and the dead cell holds under 1% of the slots, on three seeds; with the heads frozen it holds 10–18% and the loss 1.4–1.7× the floor. Routing **forced onto the three least competent experts** for 50 steps is fully undone within 150 steps of release. Two claims the economy does **not** meet, recorded as strict expected failures naming [#16](#phase-05--mechanism-correction): a 150-step forced episode drives the incumbents to the wealth ceiling and the market never re-forms (routing tracks competence at r = 0.16 against 0.84 before, on every seed), and a **ruined** competent expert stays functionally dead — its wealth climbs from the floor by ten credits in 200 steps, exploration or not, against decay.
- **The contrastive pipeline end to end** (`tests/test_contrastive_integration.py`) on an identity fake whose readout is tied to its embedding, so the diff-in-means is provably `embed[Y] − embed[X]`, injecting it provably raises the held-out log-odds, and the instruction-prefix control — whose arms both end in a full stop — is provably inert; reading the prompt instead of the completion fails the gate. PCA separability is computed and reported as a diagnostic (`behavioural_validation.pca_separability`) and read by nothing that decides.
...
| Regeneration: function re-forms after damage nobody planned for | A senescent expert is removed by its own head learning it is worth nothing; an actuator removed mid-generation leaves the consensus and the survivors carry its effort; a short forced-routing episode is undone | Measured on fixtures and the real tissue ([#6](#integration-tests-6)); a long forced episode and a ruined expert do **not** recover — the wealth band, [#16](#phase-05--mechanism-correction) |
