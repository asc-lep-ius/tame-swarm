...
- **On the real model** (`tests/test_real_model.py`) the change is not the blind cell leaving — its tail error is −0.17 σ, a seventh of the plain mean — but the re-weighting among the survivors. On the served calibration their gains run from 0.38 (cell 16) to 1.05 (cell 20), and on the replayed continuation the cells disagree by several σ on the same tokens (pushed, inert: 16 +1.5, 17 +2.9, 18 +4.5, 19 +3.6, 20 +1.2, 21 −1.6, 22 −2.4). The plain mean reads 1.21 σ inert and 0.24 live; the gain-weighted consensus reads 1.59 and 0.37 at +1.01 units of strength (−0.11 and −0.04 unpushed), the live loop moving the middle cells toward setpoint while pushing the top cells further over — the compromise one common strength can make. **How much the weighting changes the number is itself a function of the calibration**, and [#23](#calibration-corpus-23) is why this paragraph reads differently from the version before it: on the thin eight-passage corpus the survivors' gains spanned 0.48–1.68 and the two rules differed by a full σ (1.10 plain against 2.16 weighted, an 86% removal against 72%); on the served corpus the gains span 0.38–1.05 and they differ by a quarter of a σ, 80% against 77%. The re-weighting is real and it is smaller than the fixture made it look. The relative criterion holds either way; the dilution itself, a seventh of the blind error, is below the tail's 0.4 σ resolution, so the fixture and the measured plant carry that claim.
...
- **On the real model** the removal test takes the *top* actuator, below which six actuators still fire, so the rule never fires there and every survivor keeps its calibrated weight; the only change is the setpoint the tail error after the removal is measured against, now the survivors' rather than the calibrated 3.473 σ. On the served calibration that setpoint barely moves — 3.495 σ, cell 21's gain of 0.84 sitting just below the consensus gain of 0.87 — and the tail error after the removal reads +0.97 σ at 5.45 units, below the inert loop's +1.59. **The size of that shift is a fact about the calibration, not about the rule** ([#23](#calibration-corpus-23)): on the thin eight-passage corpus cell 21's gain was 39% below the consensus and the survivors' setpoint rose by 4%, which is what the earlier 5.24-against-5.05 figures recorded; at 24 passages it rises by 0.6%, and at 48 it falls by 0.4%. The rule is exercised where it bites — the wired fixture and the measured plant, below — and on the real model it is close to a null, which is the honest reading of a top removal from a tissue whose gains are nearly uniform.

<a name="calibration-corpus-23"></a>

**The calibration corpus, and what the cells' disagreement is (#23).** [#21](#blind-cell-consensus-21) made the per-cell picture visible and it showed the served tissue's cells reading one continuation several σ apart. Two readings were open: the GPU fixture calibrated on 8 prompts × 16 tokens against the server's 24 × 32, so either eight passages under-sample each cell's resting distribution and the spread is a fixture artefact, or the disagreement is the substrate's. Every real-model number in this README, and anything [#8](#phase-1--steering-economy-coupling) would quote, depended on which. `scripts/measure_calibration_corpus.py` builds the fixture once — same seed, extraction, subspaces and replayed continuation — and re-calibrates the tissue on each corpus in turn:

| corpus | passages | σ rel. SE | generate + calibrate | tissue gain | setpoint | survivors' gains | inert+push → live+push | dispersion |
|---|---:|---:|---:|---:|---:|---|---:|---:|
| 8 × 16 (the old fixture) | 8 | 27% | 6.1 + 1.5 s | 1.27 | 5.09 σ | 0.48–1.68 | +2.14 → +0.61 | 3.25 |
| 8 × 32 | 8 | 27% | 11.8 + 1.6 s | 1.15 | 4.59 σ | 0.37–1.62 | +1.63 → +0.38 | 2.86 |
| 24 × 16 | 24 | 15% | 18.1 + 4.5 s | 0.89 | 3.57 σ | 0.44–1.02 | +0.94 → +0.13 | 2.35 |
| **24 × 32 (served)** | 24 | 15% | 35.0 + 4.6 s | 0.82 | 3.30 σ | 0.37–1.00 | +0.75 → +0.10 | 2.12 |
| 48 × 32 | 48 | 10% | 69.8 + 9.2 s | 0.85 | 3.39 σ | 0.38–1.01 | +0.59 → +0.09 | 2.22 |

**The answer is both, in stated proportions.**

- **The fixture was thin, and the headline numbers were its artefact.** The slow σ is the standard deviation of the passage means, so its relative standard error is about `1 / √(2(N−1))` — 27% at eight passages — and *every* gain, setpoint and z-scored error carries it. At eight passages the middle cells' σ is under-estimated by 1.5–1.8× (cell 18: 1.70 against 2.7–2.9; 19: 1.99 against 3.4–3.7; 20: 2.84 against 3.5–3.7), so their gains were that much too high. **Passages fix it, tokens do not**: 8 × 32 moves the tissue gain by 10% and stays inside the eight-passage error bar, while 24 and 48 passages agree with each other on every cell inside their own 10–15%. The lifts, by contrast, are stable to ~1% at every corpus (16: 0.61–0.62, 18: 2.51–2.54, 22: 5.04–5.09) — what moved was never the injection's effect, only the unit it was measured in.
- **The disagreement is genuine.** At 48 passages the cells still read this one continuation from −3.2 σ (cell 22) to +3.7 σ (cell 18) under the push, and −3.9 to +2.6 unpushed. The dispersion holds at 2.0–2.3 σ in every regime and every corpus above eight passages — five times the 0.4 σ resolution of the 60-token tail — and the sign structure is fixed across corpora (16–19 below their setpoints, 20–22 above by ~3 σ). That is the content of this continuation read differently at different depths, not sampling noise. **The tissue's consensus is a compromise, not a reading**, and `dispersion` (#5) is the number that says so beside an `error` that may sit at zero. [#8](#phase-1--steering-economy-coupling) inherits it as a stated limitation.

The fixture now calibrates on the served 24 × 32 (GPU job 99 → 127 s, budget raised 120 → 160 s) and every real-model number above was re-measured on it. Two consequences worth naming. The loop is *better* than the thin corpus reported — 77% of the pushed error removed against 72% — because an over-estimated process gain had derived a slower loop (SIMC `kp` 0.64 / `ki` 0.071 against 0.94 / 0.105). And `CONTENT_PUSH` had to rise from −1.2 to −2.0 units (still inside the band's ±2 of authority): the same physical push reads in σ, and on the corrected σ the old push left the inert consensus at +0.75, under two tail standard errors.

One caveat the script records about itself: generating a calibration corpus runs the inference economy, so a corpus measured *after* another one sits on an economy that has moved. That is worth 5% of the tissue gain here (0.82 measured in sequence, 0.868 on a fixture actually built at 24 × 32), which is why the fixture's own recorded numbers come from `--corpora ""` on a fresh build and the table above is used only to compare corpora with each other.
...
- **What could falsify something.** `dispersion` (do the cells agree?), `steering_routing_correlation` (does the goal shape which experts activate?), `degenerate` (is the gate near-argmax, so that any routing claim is being read through it — #11's confound), and `/metrics/outcome` (does regulating the variable change behaviour?). Everything else — vector norms, PCA, latency, the routing histogram — is engineering telemetry, is labelled as such, and gates nothing. PCA in particular stays a diagnostic and never a quality gate: prompt-surface features separate especially cleanly ([#3](#steering-validation-3)).
...
|---|---:|---:|---:|---:|
| base (MoB only) | 65.8 ms | — | 35.9 | — |
| + steering coupling | 67.5 ms | **+2.60%** | 34.0 | −5.3% |
| + routing trace | 66.4 ms | +0.94% | 35.3 | −1.9% |
| + tissue hooks | 66.6 ms | +1.22% | 34.4 | −4.4% |
| all three (the served process) | 69.0 ms | +4.87% | 32.0 | −11.0% |

  The coupling clears #5's budget of 5% of the forward pass with room. Throughput falls further than the forward-pass percentage in every row, and that is the honest shape of it: decode is kernel-launch bound on a model this size, so a per-token addition costs more there than in a 256-token teacher-forced pass. `coupling_overhead_pct` and both throughput figures are logged to MLflow when a run is open.
...
| Metric | Healthy Range | What It Means |
|--------|---------------|---------------|
| **`dispersion`** (`/metrics/pid`) | Context, not a target | Controllability-weighted RMS of the cell errors about the consensus, in σ. **Not something to drive to zero**: 2.0–2.3 σ on the served tissue is the substrate's, not a fault ([#23](#calibration-corpus-23)). Read it beside `error` — a consensus at zero with a large dispersion means no cell is at its own setpoint |
| **`convergence_rate`** (`/metrics/pid`) | > 0 while recovering, ~0 settled | `1 − mean\|e\|_late / mean\|e\|_early` over the last 44 passes. ~0 is ambiguous on its own — read `error_rms_window` beside it: small means settled, large means the loop is not acting |
...
| **`served_minus_unsteered_log_odds`** (`/metrics/outcome`) | > `certified_random_max` | The held-out behavioural delta. At or below the random floor, steering did not help on these pairs |
...
| `/metrics/coupling` | GET | Per-MoB-layer coupling state (mode, ramp, receptor norm, the coupling's share of the perceived stream) and `steering_routing_correlation` per expert. Can say: *the goal does not shape routing* |
