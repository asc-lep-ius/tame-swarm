...
| **`steering_routing_correlation`** (`/metrics/coupling`) | Context | Per-expert correlation of a token's goal alignment with its wins. Read `correlation_basis` first: with no coupling attached this is the additive baseline, not the coupling's effect |
...
| `/metrics/pid` | GET | The tissue's consensus, every cell (weight, gain, resting σ, PID terms), its `dispersion`, and the rolling window (`convergence_rate`, `error_rms_window`). Can say: *the cells do not agree*, *the loop is not converging* |
