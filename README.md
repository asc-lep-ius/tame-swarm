...
- **Known limitation — wealth levels drain.** Correctly-priced payments are now comparable in magnitude to rewards, and they are *burned* rather than redistributed, so the economy has two sinks (payments, `wealth_decay`) against one source. Measured over 300 steps with trained heads on a specialised objective: experts are ordered by true competence markedly better than before (wealth~competence `r = +0.61`, up from `+0.37`), but 4–5 of 6 experts settle at `min_wealth` and the Gini collapses to ~0.03. Rebalancing that — redistribution, a lower decay, or a different floor — is [Phase 2](#phase-2--economy-stabilisation) and [#11](#phase-05--mechanism-correction), not a knob to turn before [#12](#phase-05--mechanism-correction) provides baselines.
