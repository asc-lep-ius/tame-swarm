...
- **The real substrate** (`tests/test_real_model.py`, `gpu`-marked, Qwen3-1.7B from the runner's cache, the served tissue and the coupling seeded at 13, 16–21). The unsteered greedy continuation of one prompt is generated once by a seeded fixture and replayed token by token under each regime with the economy frozen (identical across processes; fresh greedy generations under bf16 kernels are not, and one flipped token rewrites the tail). Content pushed against the certified direction at every actuator block, as the injection itself is applied, leaves the constant-strength loop 2.16 σ below setpoint over the last 60 tokens (0.70 σ unpushed) and the live loop at 0.61 σ with its strength raised by 0.82 units — on the real plant the criterion is relative, halve the inert error and pay for it in strength, because a 60-token tail of 9-token-filtered readings carries a standard error near 0.4 σ (these are the gain-weighted consensus of [#21](#blind-cell-consensus-21); the plain mean the tissue regulated before it read 1.10 and 0.15, for the reason given there); the top actuator removed halfway through the replay leaves the consensus, and the six survivors raise their strength from 4.00 to 5.21 and hold the error below the inert loop's (0.79 σ before, 1.03 after, against 2.16 inert); the `safe` direction passes the behavioural gate on the built-in split against eight random directions and the prefix control. The GPU job runs in 96–100 seconds on the workstation against the 120-second budget; under CI the job **fails** rather than skips when the model is not in the runner's cache, because a skipped real-model suite is a green job that ran nothing (#13), so the workstation runner must mount `~/.cache/huggingface` into the job.
...
- **On the real model** (`tests/test_real_model.py`) the change is not the blind cell leaving — its tail error is −0.24 σ, a thirtieth of the plain mean — but the re-weighting among the survivors. On this fixture's eight-prompt calibration their gains run from 0.48 (cell 16) to 1.68 (cell 19), and on the replayed continuation the cells disagree by several σ on the same tokens (pushed, inert: 16 +1.2, 17 +3.5, 18 +5.9, 19 +4.4, 20 0.0, 21 −2.9, 22 −3.1). The plain mean read 1.10 σ inert and 0.15 live only because the middle cells' deficit and the top cells' surplus cancelled; the gain-weighted consensus reads 2.16 inert and 0.61 live at +0.82 units of strength (0.70 and 0.22 unpushed), the live loop moving the high-gain middle cells toward setpoint while pushing the top cells further over — the compromise one common strength can make. The relative criterion holds with room (the live loop removes 72% of the inert error); the absolute band is the measured live error plus one tail standard error, 1.0 σ. The dilution itself, a seventh of the blind error, is below the tail's 0.4 σ resolution, so the fixture and the measured plant carry that claim.
...
- **On the real model** the removal test takes the *top* actuator, below which six actuators still fire, so the rule never fires there and every survivor keeps its calibrated weight; the only change is the setpoint the tail error after the removal is measured against, now the survivors' rather than the calibrated 5.05 σ — 5.24 σ on this fixture's calibration, cell 21's gain of 0.88 being below the consensus 1.26 — so the tail error after the removal reads +1.23 σ at 5.21 units (+1.03 against the calibrated setpoint), either way below the inert loop's +2.16.
...
About 670 tests across 37 modules covering auction properties (hand-built and hypothesis-drawn), the value definition and exploration slot, wealth dynamics, gradient checkpointing, steering, the tissue on the measured plant, recovery from designed and undesigned damage, API endpoints, config, and experts; two strict expected failures record claims the economy does not yet meet ([#6](#integration-tests-6)). `-m slow` adds the 5000-step gate-stationarity run; `-m gpu` the bitwise-determinism check and the real-model integration tests, which load Qwen3-1.7B from the local HuggingFace cache (96–100 seconds on the RTX 5070 Ti, against a 120-second budget); without the cache they skip on a developer's machine and fail under CI.
