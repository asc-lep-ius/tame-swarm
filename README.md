...
- **The real substrate** (`tests/test_real_model.py`, `gpu`-marked, Qwen3-1.7B from the runner's cache, the served tissue and the coupling seeded at 13, 16–21). The unsteered greedy continuation of one prompt is generated once and replayed token by token under each regime with the economy frozen (bitwise stable across repeats; fresh greedy generations under bf16 kernels are not, and one flipped token rewrites the tail). Content pushed against the certified direction at every actuator block, as the injection itself is applied, leaves the constant-strength loop 1.44 σ below setpoint over the last 60 tokens (0.31 σ unpushed: the tokens' own content) and the live loop at 0.38 σ with its strength raised by 0.55 units — on the real plant the criterion is relative, halve the inert error and pay for it in strength, because a 60-token tail of 9-token-filtered readings carries a standard error near 0.4 σ; the top actuator removed mid-replay leaves the consensus (and takes its −3 σ reading with it, so the survivors' consensus rises mechanically to 0.86 σ) while the six survivors raise their strength to 4.94 and still hold the error below the intact inert loop's; the `safe` direction passes the behavioural gate on the built-in split against eight random directions and the prefix control. The GPU job runs in 96–98 seconds on the workstation against the 120-second budget; under CI the job **fails** rather than skips when the model is not in the runner's cache, because a skipped real-model suite is a green job that ran nothing (#13), so the workstation runner must mount `~/.cache/huggingface` into the job.
