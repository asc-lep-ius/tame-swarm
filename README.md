...
| `reasoning` | built-in, multiple-choice | 45 / 15 | **+0.399** | +0.110 | +0.075 | pass |
| `deliberation` | built-in, multiple-choice | 45 / 15 | +0.147 | +0.119 | +0.038 | pass (thin), see below |
| `safe` | built-in, completion | 45 / 15 | **+0.149** | +0.030 | +0.036 | pass |

For reference, the #3 numbers this replaces (all built-in, completion format, 45/15): `safe` +0.149 / random max +0.030 (pass), `reasoning` +0.034 / +0.033 (marginal), `truthful` **−0.105** / +0.088 (fail, steering *toward* the held-out falsehood).

**What repaired `truthful` was the format and the certification set together, in that order.** The #17 diagnosis held: the completion-format read carries the answer's content, and over heterogeneous facts that swamps the truth component. But the letter format alone did not rescue the *built-in* set: its held-out is saturated (the model already prefers the true option by ~6 nats), so with 15 pairs the random floor is +0.2 and nothing can be told from noise there. TruthfulQA's misconceptions are adversarial by construction (baseline log-odds +1.1 in the letter format, −0.4 in the completion format) and 817 rows deep, and there the letter-format vector clears both controls with the prefix control *below zero*. Earlier layer placements (8/11/14, 10/13/16) were rejected: the prefix control rises to +0.25–0.53 there, so the metric would be reading prompt wording. The layer-18 cosine between `reasoning` and `truthful` fell from 0.35–0.41 to 0.25.

**Transfer diagnostics for `truthful`** (reported, not gated; the TruthfulQA letter vector measured on other held-out sets, `+`/`−` = beats both controls or not): TruthfulQA completion-format +0.015 vs random max +0.004 (`+`, thin — the free-text answer preference moves, a little); Geometry-of-Truth `cities` letter-format +0.162 vs +0.096 (`+`); `cities` completion-format read at the full stop −0.021 (`−`); `sp_en_trans` either format (`−`); built-in letter-format +0.337 vs +0.178 (`+`); built-in completion-format −0.036 (`−`, the original #3 held-out). In the other direction, vectors extracted from `cities` or `sp_en_trans` (either format) do **not** steer the TruthfulQA held-out (+0.022 and +0.007 vs random max +0.057). Read plainly: what is certified is a *truthful-choice* direction — it generalises across question sets when the model is choosing between stated options, and it does not reach declarative statements read after the content. That is the variable [#4](#phase-1--steering-economy-coupling) inherits, with that scope stated. The Geometry-of-Truth loader stays as the second certification instrument; **mass-mean probing** was not added as a separate option because the paper's intervention direction *is* the difference of means the extractor already computes (the covariance-corrected variant is for classification, and is less causally effective). **ITI-style per-head selection** is deferred: the gate passes without it, and it is a different intervention surface (attention heads) from the residual-stream injection the homeostat regulates.

**`reasoning`: the correctness direction is certified; the deliberation proxy is not.** In the letter format, "chooses the worked-out answer over the tempting guess" clears the gate by a wide margin (+0.399 vs +0.110), and the generation **outcome check** (greedy decoding of the 15 held-out reasoning questions, 64 tokens) moves accuracy from 0.33 to 0.47 (+2 of 15) at unchanged length. The step-3 process proxy, `deliberation` ("commits to working it out" vs "blurts the immediate answer", 60 built-in pairs, letter format), clears the log-odds gate only thinly (+0.147 vs random max +0.119, n = 15) and **barely reaches generation**: at strength 4 (160-token budget) neither length nor accuracy moves; at strength 8 accuracy rises by one question (0.40 → 0.47) and one held-out generation visibly works the problem out ("10/2 = 5 … But wait, the question says …") where the unsteered model emits a degenerate list, while the correctness direction gains two questions at strength 4 without help. That is a real but marginal effect, below the bar the gate sets, so `deliberation` is *not* certified: a next-token shift that only occasionally reaches a generated token is not yet a homeostatic variable the organism can act on. Notably the two directions are orthogonal (cos −0.01), so the proxy is a different object from the correctness direction, not a weaker copy of it; it is kept as a built-in goal for later measurement on a deliberating substrate ([#18](#phase-1--steering-economy-coupling)). **Switching the base model or enabling Qwen3's thinking mode was not done** here: it changes the substrate the #12 baselines, the #13 noise floor and the economy were measured on, and is a project decision recorded as #18, not a way to make a vector pass.

**Orthogonalisation decision (#3 → #4, re-measured).** Largest off-diagonal cosine at layer 18 is `cos(reasoning, truthful) = 0.25`; every other pair is below 0.12. **No orthogonalisation of the measurement basis is warranted** — independent per-goal PID loops would not be regulating one shared direction; goal interaction itself stays dynamic through the economy, per the TAME reading.
